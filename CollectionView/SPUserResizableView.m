...
#define kSPUserResizableViewDefaultMinHeight 5.0
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        //添加的东西
        self.stickerImage=self.stickerImage;
        self.type=self.type;
        self.hasClass=self.hasClass;
        ...
        UIPanGestureRecognizer  * panResizeGesture= [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(resizeSPTranslate:)];
        
        if (![panResizeGesture respondsToSelector:@selector(locationInView:)]) {
            
            panResizeGesture=nil;
            
        }else{
            
            
            panResizeGesture.delegate=self;
...
#pragma mark 点击手势
-(void)tapGestureRecogizer:(UITapGestureRecognizer*)recognize
{
...
#pragma mark平移手势
-(void)resizeSPTranslate:(UIPanGestureRecognizer *)recognizer
{
    ...
    switch (recognizer.state) {
        case UIGestureRecognizerStateBegan:
        {
            ...
            anchorPoint = [self anchorPointForTouchLocation:[recognizer locationInView:self]];
            
            // When resizing, all calculations are done in the superview's coordinate space.
            touchStart = [recognizer locationInView:self.superview];
            
            if (![self isResizing]) {
                
                // When translating, all calculations are done in the view's coordinate space.
                touchStart = [recognizer locationInView:self];
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            if ([self isResizing]) {
                ...
                [self resizeUsingTouchLocation:[recognizer locationInView:self.superview]];
            } else {
                
                [self translateUsingTouchLocation:[recognizer locationInView:self]];
            }
...
- (void)resizeUsingTouchLocation:(CGPoint)touchPoint {
    ...
    if (self.preventsPositionOutsideSuperview) {
        ...
        if (newY < self.superview.bounds.origin.y) {
            // Calculate how much to grow the height by such that the new Y coordintae will align with the superview.͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡͡
